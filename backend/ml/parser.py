...
nlp = spacy.load("en_core_web_sm")import re
import pdfplumber
from docx import Document
...
def extract_name(text: str) -> str:
    # Strategy: first non-empty line that is NOT an email/phone/keyword
    skip_keywords = [
    ...
    for line in lines[:8]:  # Check only top 8 lines
        line_lower = line.lower()
        # Skip lines with emails, phone numbers, URLs, or keywords
        if re.search(r"[@:/\\]|\d{7,}", line):
            continue
        if any(kw in line_lower for kw in skip_keywords):
            continue
        # Name-like: mostly letters, spaces, dots — 2 to 5 words
